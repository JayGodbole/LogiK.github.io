#!/usr/bin/env python3
"""
🎬 VIRAL SHORTS AI AGENCY — FIXED VERSION (MoviePy + imageio-ffmpeg!)
============================================================
✅ Uses MoviePy (which auto-installs FFmpeg via imageio-ffmpeg!)
✅ Generates REAL MP4 videos (not just MP3 + JPG!)
        ...
        <section id="order" class="order-section">
            ...
            <form id="orderForm">
                <div class="form-group">
                    ...
                    <textarea id="script" name="script" placeholder="Example: 'Want to make money while you sleep? Here are 3 passive income ideas that actually work...' (100-300 words)" required></textarea>
                </div>
    ...
    <script>
        ...
        document.getElementById('orderForm').addEventListener('submit', async function(e) {
            ...
            const steps = [
                ...
                '🎬 Step 2/4: Downloading background video (Pexels)...',
                '🤖 Step 3/4: Generating AI avatar (SadTalker)...',
                '🎥 Step 4/4: Rendering final video...'
            ];
...
# ==========================================
# VIDEO GENERATION FUNCTIONS (FIXED!)
# ==========================================
def install_ffmpeg():
    """Install FFmpeg using imageio-ffmpeg (for MoviePy!)"""
    try:
        import imageio_ffmpeg
        print(f"✅ imageio-ffmpeg installed! Path: {imageio_ffmpeg.get_ffmpeg_exe()}")
        return True
    except Exception as e:
        print(f"❌ imageio-ffmpeg installation failed: {e}")
        return False
...
def download_pexels_video(bg_theme, output_path="background.mp4"):
    """Download a video from Pexels API (portrait 9:16!)"""
    try:
        theme_queries = {
        ...
        if response.status_code == 200:
            ...
            if data["videos"]:
                ...
                # Pick a smallish video (faster download!)
...
def create_simple_video(voiceover_path, video_path, output_path="final_video.mp4"):
    """Create video using MoviePy (with imageio-ffmpeg for FFmpeg!)"""
    try:
        ...
        # Load video
        video = VideoFileClip(video_path).resize(RESOLUTION)
        
        # Load voiceover
        voiceover = AudioFileClip(voiceover_path)
        
        # Trim video to match voiceover duration
        video = video.subclip(0, min(video.duration, voiceover.duration))
        
        # Set audio
        final = video.set_audio(voiceover)
        ...
        final.write_videofile(
            ...
            remove_temp=True,
            ffmpeg_params=["-crf", "28"]  # Lower CRF = faster encoding!
        )
...
def generate_video():
    ...
    try:
        ...
        print("🎬 Step 2/4: Downloading background video (Pexels)...")
        background_path = download_pexels_video(payload.get("bg_theme", "Custom"))
        if not background_path:
            return jsonify({"error": "Video download failed"}), 500
        
        # Step 3: Create video (MoviePy + imageio-ffmpeg!)
        print("🎥 Step 3/4: Creating video (MoviePy + FFmpeg)...")
        final_video_path = create_simple_video(voiceover_path, background_path, video_path)
        if not final_video_path:
            return jsonify({"error": "Video rendering failed (FFmpeg error)"}), 500
        
        # Step 4: Upload
        print("☁️ Step 4/4: Uploading to free host...")
...
if __name__ == "__main__":
    print("\n" + "="*50)
    print("🚀 STARTING VIRAL SHORTS AI AGENCY (FIXED!)")
    print("="*50)
    print("✅ Uses MoviePy + imageio-ffmpeg (auto-installs FFmpeg!)")
    print("✅ Generates REAL MP4 videos (not just audio + image!)")
